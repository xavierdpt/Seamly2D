...
#include "../../../../../dialogs/tools/dialogeuler.h"
...
const QString VToolEuler::ToolType = QStringLiteral("euler");

//---------------------------------------------------------------------------------------------------------------------
/**
 * @brief VToolEuler constructor.
 ...
 */
VToolEuler::VToolEuler(VAbstractPattern *doc, VContainer *data, const quint32 &id,  const QString &typeLine,
 ...
 */
void VToolEuler::setDialog()
{
    SCASSERT(not m_dialog.isNull())
    m_dialog->setModal(true);
    QSharedPointer<DialogEuler> dialogTool = m_dialog.objectCast<DialogEuler>();
 ...
 */
VToolEuler* VToolEuler::Create(QSharedPointer<DialogTool> dialog, VMainGraphicsScene *scene, VAbstractPattern *doc,
                                   VContainer *data)
{
    SCASSERT(not dialog.isNull())
    QSharedPointer<DialogEuler> dialogTool = dialog.objectCast<DialogEuler>();
    SCASSERT(not dialogTool.isNull())
    ...
    VToolEuler *point = Create(0, pointName, typeLine, lineColor, formulaLength, formulaAngle,
 ...
 * @return the created tool
 */
VToolEuler* VToolEuler::Create(const quint32 _id, const QString &pointName, const QString &typeLine,
                                   ...
                                   const Source &typeCreation)
{
    ...
    if (parse == Document::FullParse)
    {
        VDrawTool::AddRecord(id, Tool::Euler, doc);
        VToolEuler *point = new VToolEuler(doc, data, id, typeLine, lineColor, formulaLength, formulaAngle,
 ...
 */
void VToolEuler::contextMenuEvent(QGraphicsSceneContextMenuEvent *event)
{
    try
    {
        ContextMenu<DialogEuler>(this, event);
    }
 ...
 */
void VToolEuler::SaveDialog(QDomElement &domElement)
{
    SCASSERT(not m_dialog.isNull())
    QSharedPointer<DialogEuler> dialogTool = m_dialog.objectCast<DialogEuler>();
    SCASSERT(not dialogTool.isNull())
...
//---------------------------------------------------------------------------------------------------------------------
void VToolEuler::SaveOptions(QDomElement &tag, QSharedPointer<VGObject> &obj)
{
...
//---------------------------------------------------------------------------------------------------------------------
void VToolEuler::ReadToolAttributes(const QDomElement &domElement)
{
...
//---------------------------------------------------------------------------------------------------------------------
void VToolEuler::SetVisualization()
{
    if (not vis.isNull())
    {
        VisToolEuler *visual = qobject_cast<VisToolEuler *>(vis);
...
//---------------------------------------------------------------------------------------------------------------------
VFormula VToolEuler::GetFormulaAngle() const
{
...
//---------------------------------------------------------------------------------------------------------------------
void VToolEuler::SetFormulaAngle(const VFormula &value)
{
...
//---------------------------------------------------------------------------------------------------------------------
void VToolEuler::ShowVisualization(bool show)
{
    ShowToolVisualization<VisToolEuler>(show);
}
